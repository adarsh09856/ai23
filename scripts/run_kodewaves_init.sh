...
if [[ "${ENVIRONMENT:-local}" == "production" ]]; then
    ...
    dograh_success "✓ kodewaves-init rendered remote nginx and coturn config"
    exit 0
fi
...
if [[ -n "${TURN_SECRET:-}" && -n "${TURN_HOST:-}" ]]; then
    ...
    dograh_render_remote_turn_conf "$WORKSPACE_DIR" "$COTURN_OUTPUT_DIR/turnserver.conf"
    dograh_success "✓ kodewaves-init rendered local TURN config"
    exit 0
fi

dograh_success "✓ kodewaves-init no-op for current profile"
